...
const SectorsFilterPage = () => {
  ...
  useEffect(() => {
    dispatch(getPaginatedData({ page: CurrentPage, sector: Sector }));
  }, [CurrentPage, Sector]);

  const sectorsFilterOptions =
    allData?.allSectors?.length > 0 && allData?.allSectors;

  console.log({ sectorsFilterOptions });

  const options = sectorsFilterOptions?.length > 0 && sectorsFilterOptions;
      ...
      <div className="w-full flex justify-start items-center dark:text-white dark:bg-blue-950 rounded-md">
        {options?.length > 0 && (
          <Autocomplete
            ...
            options={options}
            renderInput={(params) => (
              <TextField {...params} label="filter data by Sectors" />
            )}
          />
        )}
      </div>
...
export default SectorsFilterPage;
