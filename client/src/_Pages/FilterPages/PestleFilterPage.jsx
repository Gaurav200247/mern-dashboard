...
const PestleFilterPage = () => {
  ...
  useEffect(() => {
    dispatch(getPaginatedData({ page: CurrentPage, pestle: Pestle }));
  }, [CurrentPage, Pestle]);

  const PestlesFilterOptions =
    allData?.allPestles?.length > 0 && allData?.allPestles;

  console.log({ PestlesFilterOptions });

  const options = PestlesFilterOptions?.length > 0 && PestlesFilterOptions;

  const columns = [
    { field: "_id", headerName: "ID", width: 230 },
    { field: "title", headerName: "Title", width: 300 },
    { field: "source", headerName: "Sources", width: 230 },
    { field: "pestle", headerName: "Pestles", width: 130 },
        ...
        {options?.length > 0 && (
          <Autocomplete
            ...
            onInputChange={(event, newInputValue) => {
              setPestle(newInputValue);
            }}
            id="controllable-states-demo"
            options={options}
            renderInput={(params) => (
              <TextField {...params} label="filter data by Pestles" />
            )}
          />
        )}
      </div>
...
export default PestleFilterPage;
